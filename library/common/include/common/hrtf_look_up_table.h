...
constexpr auto az_inc{find_inc(std::begin(hrtf_data::entries), std::end(hrtf_data::entries), azimuth{})};
constexpr auto el_inc{find_inc(std::begin(hrtf_data::entries), std::end(hrtf_data::entries), elevation{})};

constexpr auto az_num{360 / az_inc};
constexpr auto el_num{360 / el_inc};

constexpr size_t index_from_azimuth(int az) {
    return (az % 360) / az_inc;
}

constexpr int azimuth_from_index(size_t ind) {
    return ind * az_inc;
}

constexpr size_t index_from_elevation(int el) {
    return (el % 360) / el_inc;
}

constexpr int elevation_from_index(size_t ind) {
    return ind * el_inc;
}

constexpr auto energy_for_angles(
        const hrtf_data::entry* it,
        const hrtf_data::entry* end,
        int azimuth,
        int elevation) {
    for (; it != end; ++it) {
...
constexpr auto energy_for_indices(
        const hrtf_data::entry* it,
        const hrtf_data::entry* end,
        size_t az,
        size_t el) {
    return energy_for_angles(it,
                             end, 
                             azimuth_from_index(az),
                             elevation_from_index(el));
}
...
constexpr auto generate_look_up_array(const hrtf_data::entry* it,
                                     const hrtf_data::entry* end,
                                     size_t azimuth_index) {
    return generate_look_up_array(it,
                                 end, 
                                 azimuth_index, 
                                 std::make_index_sequence<el_num>{});
}
...
constexpr auto generate_look_up_table(const hrtf_data::entry* it,
                                     const hrtf_data::entry* end) {
    return generate_look_up_table(it,
                                 end,
                                 std::make_index_sequence<az_num>{});
}

constexpr auto look_up_table{generate_look_up_table(std::begin(hrtf_data::entries), std::end(hrtf_data::entries))};

constexpr auto look_up_angles(int azimuth, int elevation) {
    return look_up_table[index_from_azimuth(azimuth)][index_from_elevation(elevation)];
}
