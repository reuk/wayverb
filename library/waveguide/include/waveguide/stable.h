...
constexpr auto make_next_array(T&& a, double rci, std::index_sequence<Ix...>) {
    ...
    return std::array<double, size>{{(tuple_like_getter<Ix>(a) -
                                      rci * tuple_like_getter<size - Ix>(a)) /
                                     (1 - rci * rci)...}};
}

template <typename T>
constexpr auto make_next_array(T&& a, double rci) {
    constexpr auto size = tuple_like_size_v<decay_const_ref_t<T>>;
...
template <
        typename T,
        std::enable_if_t<tuple_like_size_v<decay_const_ref_t<T>> == 1, int> = 0>
...
constexpr auto is_stable(T&& a) {
    constexpr auto size = tuple_like_size_v<decay_const_ref_t<T>>;
    const auto rci = tuple_like_getter<size - 1>(a);
    if (1 <= std::abs(rci)) {
        return false;
    }
