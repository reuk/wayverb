...
void ImpulseRoutingComponent::paint(Graphics& g) {
    ...
    AngularLookAndFeel::matte_box(
            g,
            bounds,
            false,
            drag ? AngularLookAndFeel::emphasis : Colours::darkgrey);
    bounds.reduce(4, 4);
...
std::vector<model::CarrierRouting> init_carrier_channels(int c,
    ...
    for (auto i = 0u; i != c; ++i) {
        ...
        ret.emplace_back(model::CarrierRouting{
                ss.str(),
                std::vector<model::CarrierRouting::my_bool>(output_channels,
                                                            0)});
    }

    //  by default, we'll just send the first channel to all outputs
    std::fill(ret.front().channel.begin(), ret.front().channel.end(), 1);
    return ret;
}
...
private:
    ...
    public:
        ...
        void paint(Graphics& g) override {
            g.setColour(isMouseOver() ? AngularLookAndFeel::emphasis
                                      : Colours::lightgrey);
