...
AsyncEngine::SingleShotEngineThread::SingleShotEngineThread(
        AsyncEngine& listener,
        ...
        , thread([this,
            ...
            try {
                ...
                std::cerr << "impulses estimated: " << impulses << std::endl;
                    ...
                    if (!valid) {
                        NativeMessageBox::showMessageBoxAsync(
                                ...
                                "It looks like that " + str +
                                        " position is outside the waveguide "
                ...
                std::cout << "wayverb thread error: " << e.what() << std::endl;
            } catch (...) {
                std::cout << "something went pretty wrong so the render thread "
                             "is qutting now"
                          << std::endl;
            }
...
AsyncEngine::SingleShotEngineThread::~SingleShotEngineThread() noexcept {
    keep_going = false;
}
...
void AsyncEngine::start(const model::Persistent& wrapper,
                        const CopyableSceneData& scene_data,
                        bool visualise) {
    std::lock_guard<std::mutex> lck(mut);
    thread = std::make_unique<SingleShotEngineThread>(
            *this, wrapper, scene_data, visualise);
}
