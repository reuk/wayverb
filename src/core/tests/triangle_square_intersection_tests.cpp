...
TEST(triangle_square_intersection, perp) {
    ASSERT_EQ(geo::detail::perp(glm::vec2(0, 1)),
              glm::vec2(-1, 0));
    ASSERT_EQ(geo::detail::perp(glm::vec2(2, 2)),
              glm::vec2(-2, 2));
    ASSERT_EQ(geo::detail::perp(glm::vec2(-4, 2)),
              glm::vec2(-2, -4));
}

TEST(triangle_square_intersection, normal_2d) {
    ASSERT_EQ(geo::detail::normal_2d(glm::vec2(0, 0),
                                                    glm::vec2(0, 1)),
              glm::vec2(-1, 0));
    ASSERT_EQ(geo::detail::normal_2d(glm::vec2(0, 0),
                                                    glm::vec2(2, 2)),
...
TEST(triangle_square_intersection, normals_2d) {
    ...
    geo::detail::normals_2d(
            to_test.begin(), to_test.end(), out.begin());
    ASSERT_EQ(out,
