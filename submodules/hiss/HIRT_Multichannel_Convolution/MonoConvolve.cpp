...
void *largest_partition_time_alloc(AH_UIntPtr size, AH_UIntPtr nominalSize)
{
    return new HISSTools::DSP::PartitionedConvolve(16384, (size > 16384 ? size : 16384) - 8192, 8192, 0);
}


void *largest_partition_fft1_alloc(AH_UIntPtr size, AH_UIntPtr nominalSize)
{
	return new HISSTools::DSP::PartitionedConvolve(16384, (size > 16384 ? size : 16384) - 8064, 8064, 0);
}

void *largest_partition_fft2_alloc(AH_UIntPtr size, AH_UIntPtr nominalSize)
{
	return new HISSTools::DSP::PartitionedConvolve(16384, (size > 16384 ? size : 16384) - 7680, 7680, 0);
}


void largest_partition_free(void *largePartition)
{
	delete (HISSTools::DSP::PartitionedConvolve *) largePartition;
}
...
HISSTools::DSP::MonoConvolve::MonoConvolve(uintptr_t maxLength, t_convolve_latency_mode latency)
{
	long fail = 0;
...
HISSTools::DSP::MonoConvolve::~MonoConvolve()
{
    delete mTime1;
    delete mPart1;
    delete mPart2;
...
HISSTools::DSP::PartitionedConvolve *HISSTools::DSP::MonoConvolve::resize(uintptr_t length, bool keep_lock)
{
...
t_convolve_error HISSTools::DSP::MonoConvolve::set(const float *input, uintptr_t length, bool resizeFlag)
{	
...
t_convolve_error HISSTools::DSP::MonoConvolve::reset()
{
    AH_UIntPtr max_impulse;
...
void HISSTools::DSP::MonoConvolve::sum(float *out, float *add, uintptr_t numSamples)
{
...
void HISSTools::DSP::MonoConvolve::process(float *in, float *temp, float *out, uintptr_t numSamples)
{		
	AH_UIntPtr max_impulse = 0;
	PartitionedConvolve *part4 = (PartitionedConvolve *) attempt_memory_swap(&mPart4, &max_impulse);
	
	// N.B. This function DOES NOT zero the output buffer as this is done elsewhere
	
	if (mLength && mLength <= max_impulse && part4)
	{
		if (mLatency == 0)
		{
