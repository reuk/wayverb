...
namespace HISSTools
{
    namespace DSP
    {
        ...
        public:
            ...
            // FIX - check if these constuctors are ambiguous - I supect they are, so
            // imagine another way....
            ...
            void process(const double* const* const ins, double* const* const outs,
                         size_t numIns, size_t numOuts, size_t numSamples);
            void process(const std::vector<const double*>& ins,
                         const std::vector<double*>& outs, size_t numSamples);
