...
HISSTools::DSP::Convolver::Convolver(uint32_t numIns, uint32_t numOuts, LatencyMode latency)
{
    ...
    {
        mConvolvers.push_back(new HISSTools::DSP::NToMonoConvolve(numIns, 16384, (t_convolve_latency_mode) latency));
...
HISSTools::DSP::Convolver::Convolver(uint32_t numIO, LatencyMode latency)
{
    ...
    {
        mConvolvers.push_back(new NToMonoConvolve(1, 16384, (t_convolve_latency_mode) latency));
        mInTemps.push_back(NULL);
    }
...
HISSTools::DSP::Convolver::~Convolver()
{
...
t_convolve_error HISSTools::DSP::Convolver::resize(uint32_t inChan, uint32_t outChan, uintptr_t length)
{
    // For Parallel operation you must pass the same in/out channel
...
t_convolve_error HISSTools::DSP::Convolver::set(uint32_t inChan, uint32_t outChan, const float *input, uintptr_t length, bool resize)
{
    // For Parallel operation you must pass the same in/out channel
...
t_convolve_error HISSTools::DSP::Convolver::set(uint32_t inChan, uint32_t outChan, const double *input, uintptr_t impulseLength, bool resize)
{
    float *inputFloat = new float[impulseLength];
    
    for (unsigned long i; i < impulseLength; i++)
...
void HISSTools::DSP::Convolver::process(double **ins, double **outs, uint32_t numIns, uint32_t numOuts, uintptr_t numSamples)
{
    setMXCSR();
    
    void *memPointer = grow_memory_swap(&mTemporaryMemory, (mNumIns + 2) * numSamples * sizeof(float), numSamples);
    ...
    for (uintptr_t i = 0; i < numIns; i++)
        for (AH_UIntPtr j = 0; j < numSamples; j++)
            mInTemps[i][j] = (float) ins[i][j];
    
    // FIX - nasty!
    
    for (AH_UIntPtr i = 0; i < numOuts; i++)
        mConvolvers[i]->process(mN2M ? &mInTemps[0] : (&mInTemps[0]) + i, outs[i], mTemp1, mTemp2, numSamples, numIns);
...
void HISSTools::DSP::Convolver::process(std::vector<double *>& ins, std::vector<double *>outs, uintptr_t numSamples)
{
    process(&ins[0], &outs[0], ins.size(), outs.size(), numSamples);
}
