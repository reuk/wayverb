...
HISSTools::DSP::TimeDomainConvolve::TimeDomainConvolve(uintptr_t offset, uintptr_t length) : mInputPosition(0), mImpulseLength(0)
{
...
HISSTools::DSP::TimeDomainConvolve::~TimeDomainConvolve()
{
    ALIGNED_FREE(mImpulseBuffer);
    ALIGNED_FREE(mInputBuffer);
}

void HISSTools::DSP::TimeDomainConvolve::setOffset(uintptr_t offset)
{
	mOffset = offset;
}

t_convolve_error HISSTools::DSP::TimeDomainConvolve::setLength(uintptr_t length)
{	
...
t_convolve_error HISSTools::DSP::TimeDomainConvolve::set(const float *input, uintptr_t length)
{
	t_convolve_error error = CONVOLVE_ERR_NONE;
...
void HISSTools::DSP::TimeDomainConvolve::reset()
{
    mReset = true;
}
...
void convolve(float *in, vFloat *impulse, float *output, uintptr_t N, uintptr_t L)
{
	vFloat output_accum;
	float *input;
	float results[4];
...
void convolve(float *in, float *impulse, float *output, uintptr_t N, uintptr_t L)
{
	float output_accum;
	float *input;
...
void HISSTools::DSP::TimeDomainConvolve::process(float *in, float *out, uintptr_t numSamples)
{
    if (mReset)
    {
        memset(mInputBuffer, 0, 8192 * sizeof(float));
...
void HISSTools::DSP::TimeDomainConvolve::processScalar(float *in, float *out, uintptr_t numSamples)
{
...
void HISSTools::DSP::TimeDomainConvolve::processSIMD(float *in, float *out, uintptr_t numSamples)
{
	uintptr_t currentLoop;
