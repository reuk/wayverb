...
constexpr auto make_slope_array(double begin,
    ...
    return std::array<double, sizeof...(Ix)>{{wayverb::core::linear_interp(
            Ix / (sizeof...(Ix) - 1.0), begin, end)...}};
}
...
int main(int argc, char** argv) {
    ...
    const auto make_reflectance_coefficients = [&](const auto& absorption) {
        return wayverb::waveguide::compute_reflectance_filter_coefficients(
                absorption, sample_rate);
    };

    const auto reflectance = std::vector<
            util::named_value<wayverb::waveguide::coefficients_canonical>>{
            {"sloping_fitted_0",
             make_reflectance_coefficients(
                     make_slope_array<wayverb::core::simulation_bands>(0, 1))},

            {"sloping_fitted_1",
             make_reflectance_coefficients(
                     make_slope_array<wayverb::core::simulation_bands>(1, 0))},
