...
bool voxel_inside(float3 pt,
                  ...
                  const global Triangle* triangles,
                  const global float3* vertices) {
    const Ray ray = {pt, (float3)(0, 0, 1)};
    uint count = 0;

    VOXEL_TRAVERSAL_ALGORITHM({
        for (uint i = 0; i != num_triangles; ++i) {
            const float distance = triangle_intersection(
                    triangles + voxel_begin[i], vertices, ray);
            if (EPSILON < distance && distance < max_dist_inside_voxel) {
                count += 1;
            }
        }
    })
...
bool voxel_point_intersection(float3 begin,
    ...
    Intersection inter = voxel_traversal(to_point,
                                         voxel_index,
                                         global_aabb,
                                         side,
                                         triangles,
