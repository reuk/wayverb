...
uint single_ray_inside(ray r,
                       ...
                       const global triangle* triangles,
                       const global float3* vertices) {
    const uint intersections = count_intersections(r, voxel_index, global_aabb, side, triangles, vertices);
...
constant ulong num_random_directions = sizeof(random_directions) / sizeof(float3);
