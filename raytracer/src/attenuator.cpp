...
hrtf::hrtf(const compute_context& cc, double speed_of_sound)
        : queue(cc.context, cc.device)
        , kernel(attenuator_program{cc, speed_of_sound}.get_hrtf_kernel())
        , cl_hrtf(cc.context, CL_MEM_READ_WRITE, sizeof(volume_type) * 360 * 180) {
}
...
microphone::microphone(const compute_context& cc, double speed_of_sound)
        : queue(cc.context, cc.device)
        , kernel(attenuator_program{cc, speed_of_sound}
