...
kernel void raytrace_improved(global RayInfo * ray_info,
                              const global uint * voxel_index,
                              AABB global_aabb,
                              int side,
                              const global Triangle * triangles,
    ...
    Intersection closest =
        ray_triangle_intersection(ray, triangles, numtriangles, vertices);
#endif
    ...
    if (index < NUM_IMAGE_SOURCE) {
        ...
        for (ulong k = 0; k != index + 1 && intersects; ++k) {
            ...
            for (long l = k - 1; l != -1; --l) {
                intersection_point =
                    mirror_point(intersection_point, info->prev_primitives[l]);
            }
            ...
            Intersection inter = ray_triangle_intersection(
                intermediate, triangles, numtriangles, vertices);
#endif
        ...
        if (intersects) {
            ...
            intersects = point_intersection(
                prev_intersection, mic, triangles, numtriangles, vertices);
#endif
        }
    ...
    const bool is_intersection = point_intersection(
        intersection, mic, triangles, numtriangles, vertices);
#endif
...
kernel void attenuate(float3 mic_pos,
